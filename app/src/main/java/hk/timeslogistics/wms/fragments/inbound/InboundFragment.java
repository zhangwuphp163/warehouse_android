...
public class InboundFragment extends Fragment {
    ...
    //protected Button buttonScanDetail;

    private CredentialManager credentialManager;
    private EditText editTextAsnNumber;
    private EditText editTextPoNumber;
    private EditText editTextLocation;
    private View editTextQtyView;
    private EditText editTextInboundQty;
    private EditText editTextSkuBarcode;
    private EditText editTextBoxId;
    private LinearLayout linearLayoutQty;
    private Spinner spinnerCondition;
    //private LinearLayout listViewLine;
    private LinearLayout linearLayoutBoxId;
    private int is_require_piece_scan = 0;
    private String is_scan_box_id = "0";
    private String box_id_single_sku = "1";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        @SuppressLint("InflateParams") ViewGroup root = (ViewGroup) inflater.inflate(R.layout.fragment_inbound, null);

        Objects.requireNonNull(((AppCompatActivity) getActivity()).getSupportActionBar()).setTitle(R.string.nav_inbound);
        ...
        editTextInboundQty = (EditText) root.findViewById(R.id.editTextInboundQty);
        editTextBoxId = (EditText) root.findViewById(R.id.editTextBoxId);
        editTextQtyView = (View) inflater.inflate(R.layout.qty,null);
        linearLayoutQty = (LinearLayout) root.findViewById(R.id.linearLayoutQty);
        ...
        editTextInboundQty.setShowSoftInputOnFocus(false);
        ...
        buttonSubmit = (Button) root.findViewById(R.id.buttonReceivedComplete);
        linearLayoutBoxId = (LinearLayout) root.findViewById(R.id.linearLayoutBoxid);
        ...
        buttonSubmit.setOnClickListener(v -> {
            ...
            builder.setTitle(R.string.nav_inbound)
                    .setCancelable(true)
                    .setMessage("Confirm Received Complete?\n确认收货完成？")
        ...
        buttonScanAsnNumber.setOnClickListener(v -> {
            ...
            IntentIntegrator.forSupportFragment(InboundFragment.this).initiateScan();
        });
    ...
    public boolean checkBoxIdIsSingleSku(String barcode) throws JSONException {
        boolean is_same = true;
        if(is_scan_box_id.equals("0") && box_id_single_sku.equals("1")){
            for (int i =0;i<inboundJsonArray.length();i++){
                JSONObject localJsonObject = (JSONObject) inboundJsonArray.get(i);
                String sku_barcode = localJsonObject.get("sku_barcode").toString();
                if(!barcode.equals(sku_barcode)){
                    is_same = false;
                    break;
                }
            }
    ...
    public void setList() throws JSONException {
        ...
        String location;
        String inbound_qty = editTextInboundQty.getText().toString();
        if(po_number.isEmpty() || sku_barcode.isEmpty()){
            return;
        }
        ...
        scanJsonObject.put("condition",condition);
        scanJsonObject.put("qty",inbound_qty);
        ...
        /*for (int i=0;i< itemsJsonArray.size();i++){
            ...
            if(po_number.equals(itemPoNumber)){
                boolean is_exists = InboundModel.checkSkuBarcodeInAsn(item,sku_barcode);
                if(is_exists){
                    if(item.has("scan_qty")){
                        int scan_qty = item.get("scan_qty").getAsInt();
                        int qty = Integer.parseInt(inbound_qty);
                        scan_qty += qty;
                        item.addProperty("scan_qty",scan_qty);
                    }else{
                        item.addProperty("scan_qty",1);
                    }
                    item = itemsJsonArray.get(i).getAsJsonObject();
                    itemsJsonArray.remove(i);
                    JsonArray newJsonArray = new JsonArray();
                    newJsonArray.add(item);
                    for(int j=0;j < itemsJsonArray.size();j++){
                        newJsonArray.add(itemsJsonArray.get(j).getAsJsonObject());
                    }
                    itemsJsonArray = newJsonArray;
                    break;
                }
            }
        }*/
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == IntentIntegrator.REQUEST_CODE) {
            if (resultCode == InboundFragment.RESULT_OK) {
    ...
    public void resetFields() {
        ...
        editTextInboundQty.setText("1");
        if(is_scan_box_id.equals("1")){
            linearLayoutBoxId.setVisibility(View.VISIBLE);
        }else{
            linearLayoutBoxId.setVisibility(View.GONE);
        }
        if(is_require_piece_scan == 1){
            linearLayoutQty.setVisibility(View.VISIBLE);
        }else{
            linearLayoutQty.setVisibility(View.GONE);
        }
    ...
    private void submit(String is_force_receive) throws JSONException {
        ...
        requestData.put("location_type","bin");
        requestData.put("boxid",boxid);
    ...
    private void processCheckAsn(RemoteResult result){
        if (result.getStatus().equals(ErrorHandler.STATUS_SUCCESS)) {
            ...
            is_require_piece_scan = jsonObject.get("is_require_piece_scan").getAsInt();
            itemsJsonArray = result.getItems().getAsJsonArray();
            is_scan_box_id = jsonObject.get("is_scan_box_id").getAsString();
            box_id_single_sku = jsonObject.get("box_id_single_sku").getAsString();
    ...
    private void setFieldRequestFocus(){
        ...
        if(is_scan_box_id.equals("1") && boxid.isEmpty()){
        ...
        try{
            boolean  is_same = checkBoxIdIsSingleSku(sku_barcode);
            if(!is_same){
                TomAlertDialog.showAlertDialogMessage(getContext(),"The box can't hold different SKUs");
                editTextSkuBarcode.setText("");
                editTextSkuBarcode.requestFocus();
                return;
            }
        ...
        if(is_require_piece_scan == 0){
            EditText editTextQty = (EditText) editTextQtyView.findViewById(R.id.qty);
            editTextQty.setText("");
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            Dialog dialog = builder.setTitle("入庫數量").setIcon(R.drawable.checked)
                    .setView(editTextQtyView)
                    .setCancelable(false)
                    .setPositiveButton(getString(R.string.button_confirm), (dialogInterface, i) -> {
                        String qty = editTextQty.getText().toString();
                        InputMethodManager imm = (InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                        assert imm != null;
                        imm.toggleSoftInput(1,InputMethodManager.HIDE_NOT_ALWAYS);

                        editTextInboundQty.setText(qty);
                        ViewGroup parent = (ViewGroup) editTextQtyView.getParent();
                        if(parent != null){
